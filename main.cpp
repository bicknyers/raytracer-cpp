...
hittable_list random_scene() {
    ...
    // Color for Ruby
    auto material2 = make_shared<metal>(color(0.878, 0.067, 0.373), 0.0);
    auto mesh = read_mesh("gem.obj", &world, material2, 1.5, point3(0,0,0));
    mesh.add_mesh();
    ...
    for (int a = -11; a < 11; a++) {
        for (int b = -11; b < 11; b++) {
            if (a*b < 1 && -1 < a*b) { // Clear origin for Gem
            } else {
...
int image_width = 384;//480
int samples_per_pixel = 40;//60
int max_depth = 16;//24
int num_threads = 4;

if (argc >= 2) {
  image_width = static_cast<int>(clamp(stoi(argv[1]), 480, 3840));
  if (argc >= 3) {
    samples_per_pixel = static_cast<int>(clamp(stoi(argv[2]), 60, 1000));
    if (argc >= 4) {
      max_depth = static_cast<int>(clamp(stoi(argv[3]), 24, 500));
      if (argc >= 5) {
